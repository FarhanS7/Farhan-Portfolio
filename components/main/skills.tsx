...
import { BarChart, Cloud, Code2, Database, Search, Star } from "lucide-react";
import Image from "next/image";
import React, { useRef, useState } from "react";
import { Skill, skills } from "../../constants/index";
...
const SkillCard: React.FC<{ skill: Skill; index: number }> = ({ skill }) => {
  const { isDark } = useTheme();
  const [isHovered, setIsHovered] = useState(false);
  const cardRef = useRef(null);
  const isInView = useInView(cardRef, { once: false, margin: "-100px" });
  const years =
    new Date().getFullYear() - (skill.yearStarted || new Date().getFullYear());
      ...
      <div className="container mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        <div className="mb-12 flex flex-col sm:flex-row gap-6 items-center justify-between">
          <div className="flex flex-wrap justify-center gap-3">
            {(
              ["all", "frontend", "backend", "database", "devops"] as Category[]
            ).map((category) => (
              <motion.button
                key={category}
          ...
          <div className="flex gap-4 items-center">
            ...
            <motion.button
              ...
              whileTap={{ scale: 0.95 }}
              onClick={() =>
                setViewMode(viewMode === "grid" ? "stats" : "grid")
              }
              className={`
                p-2.5 rounded-full
                ${isDark ? "bg-gray-800/50" : "bg-white/50"}
                  ...
                  >
                    ...
                    <div className="h-3 bg-gray-200/10 rounded-full overflow-hidden">
                      <motion.div
                        initial={{ width: 0 }}
                        animate={{
                          width: `${(count / filteredSkills.length) * 100}%`,
                        }}
